...
parser = argparse.ArgumentParser(
    description="per frame testing: video-level curve requirement")
parser.add_argument('dataset', type=str, choices=['mm','ucf101', 'hmdb51', 'kinetics','thumos14'])
...
data_loader = torch.utils.data.DataLoader(
        PerFrameData(args.frames_root, args.test_list, num_segments=args.test_segments,
...
def eval_video(video_data):
    ...
    return rst.reshape((num_crop, -1, num_class)).mean(axis=0).reshape(
        (-1, num_class) #defualt as args.test_segments=1
    ), label
...
#output=np.array([x[0] for x in output]).reshape(-1,num_class)
